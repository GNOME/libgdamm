...
int main (int argc, char** argv)
{
  ...
  const Glib::ustring database_name = "template1"; //You can't connect to postgres without specifying a database. "template1" always exists.

  Glib::RefPtr<Gnome::Gda::Client> gda_client = Gnome::Gda::Client::create();
  if(gda_client)
  {
    //Get a stored data source:
    const Glib::ustring data_source_name = "datasource_libgdamm_example_simple3";
    ...
    if(!data_source)
    {
      ...
      data_source.set_cnc_string("DATABASE=" + database_name); //You can't connect to postgres without specifying a database.
    ...
    else
    {
      ...
      //gda_connection->change_database("tblTest1"); //This does not work with postgres - you can only specify a database when opening a connection.
      ...
      {
        std::cout << " libgda reported 0 tables for database " << database_name << std::endl;
      }
